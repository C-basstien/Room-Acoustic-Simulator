function parallel_realtime(hObject, eventdata, handles)
%%function parallel_realtime(hObject, eventdata, handles)
%not supported now
%%
...
if(b_sz(2)<2)
    if (h.ptr_audio<h.maxlength)
        ...
        h.buffersize=floor(h.totalchnl*(mean(h.room.t60)+0.5)*44100);%emprical estimatiom pf  the needed computation time 0.5 stand for the minimal amount of time to generate a room...
        h.chg=p.fig.chg;
        ...
        for i= 1:h.filenum-1
            ...
            for k= 1:h.sounds{i}.nbchnl
                ...
                if (k == 1 && i==1 && h.ptr_audio==1)
                    sz=[h.maxlength+20*44100,2];%to initialize the ouput, assuming that the ouput lenght is below maxlenght+twenty s, true for a realisitc simulation
                    h.out_res=zeros(sz);
